...
export interface IStorage {
  ...
  // Product operations
  getProducts(limit?: number, offset?: number, filters?: ProductFilters): Promise<Product[]>;
  ...
  getPurchaseReport(startDate: Date, endDate: Date): Promise<any[]>;
  getStockReport(): Promise<{ lowStockItems: Product[], totalValue: number }>;

  getSalesDetails(startDate: Date, endDate: Date): Promise<Order[]>;
  getPurchaseDetails(startDate: Date, endDate: Date): Promise<Purchase[]>;
  getStockDetails(): Promise<ProductWithCategory[]>;
  ...
  updateOrderPathaoInfo(id: number, info: { consignmentId: string, merchantOrderId: string, status: string }): Promise<Order | undefined>;
}
...
export class MemStorage implements IStorage {
  ...
  async getSalesReport(startDate: Date, endDate: Date): Promise<any[]> {
    // Basic in-memory implementation - mostly for fallback
    return [];
  }
