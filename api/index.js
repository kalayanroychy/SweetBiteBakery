...
var categories, insertCategorySchema, products, insertProductSchema, users, insertUserSchema, orders, insertOrderSchema, orderItems, insertOrderItemSchema, cartSchema, loginSchema, registerSchema, newsletterSchema, contactFormSchema;
  ...
  "node_modules/drizzle-orm/neon-serverless/session.js"() {
    ...
    NeonSession = class _NeonSession extends PgSession {
      ...
      async transaction(transaction, config = {}) {
        const session2 = this.client instanceof Pool ? new _NeonSession(await this.client.connect(), this.dialect, this.schema, this.options) : this;
...
var init_db2 = __esm({
  "server/db.ts"() {
    ...
    pool = DATABASE_URL ? new Pool3({
      connectionString: DATABASE_URL,
      max: 10,
      idleTimeoutMillis: 3e4,
      // Reduced to 30s
      connectionTimeoutMillis: 1e4
      // Reduced to 10s for faster failure/fallback
    }) : null;
    if (pool) {
      pool.on("error", (err) => {
...
var DatabaseStorage = class {
  ...
  async updateCategory(id, category) {
    const [updatedCategory] = await getDb().update(categories).set(category).where(eq(categories.id, id)).returning();
  ...
  async getProductsByCategory(categoryId) {
    return await getDb().select().from(products).where(eq(products.categoryId, categoryId)).orderBy(desc(products.createdAt));
  }
  async getFeaturedProducts() {
    return await getDb().select().from(products).where(eq(products.featured, true)).orderBy(desc(products.createdAt)).limit(6);
  }
  ...
  async updateProduct(id, product) {
    const [updatedProduct] = await getDb().update(products).set(product).where(eq(products.id, id)).returning();
